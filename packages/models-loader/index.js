...
const register = async function (server, options) {
  ...
  server.ext({
    ...
    method: async function (_server) {
      ...
      server.log(['info', 'mongodb'], 'AngaModelsLoader: finished processing auto indexes.');
    }
  });
  ...
  server.ext({
    ...
    method: function (_server) {
      ...
      server.log(['info', 'mongodb'], 'AngaModelsLoader: closed db connection(s).');
    }
  });

  await MongoModels.connect(options.mongodb.connection, options.mongodb.options);

  server.log(['info', 'mongodb'], 'AngaModelsLoader: successfully connected to the db.');
};
